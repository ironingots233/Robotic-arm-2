...
POLICY_PATH = "/home/hzy/mybot/output_lerobot_train/cocacola/checkpoints/020000/pretrained_model"
...
def run_inference():
    print("=== ACT Inference Deployment ===")
    ...
    policy = ACTPolicy.from_pretrained(POLICY_PATH, config=cfg)
    policy.to(DEVICE)
    policy.eval()
    policy.reset()
    print("Policy loaded.")
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                print(f"\r⚠️ [硬件警告] 跳过当前帧: {e}{' '*10}", end="")
                continue
            ...
            if a.shape != (6,) or np.isnan(a).any() or np.isinf(a).any():
                print(f"\r⚠️ [策略警告] 非法动作，跳过: {a}{' '*10}", end="")
                continue
